...
void getRandomAddress(pointer *virt, pointer *phys) {
    size_t offset = (size_t)(rand() % (mapping_size / 32)) * 32;
...
std::vector<double> prob_function(std::vector <pointer> masks, int align_bit) {
    ...
    for (std::vector<pointer>::iterator it = masks.begin(); it != masks.end();
        ...
        //std::cout<<"PROB "<<count<<" "<<sets.size()<<std::endl;
        prob.push_back((double) count / sets.size());
    }
    return prob;
}

size_t findSet(pointer phys_addr)
{
...
int main(int argc, char *argv[]) {
    ...
    std::set <addrpair> mysets[numSets];
    ...
    for (std::set<addrpair>::iterator it = addr_pool.begin() ; it != addr_pool.end() ; it ++)
    {
        //printf("PAIR: %d %d\n", it->first, it->second);
        size_t set = findSet((*it).second);
        //printf("%d\n", set);
        mysets[set].insert(*it);
    }
	...
	setNo = 15;
        timing.clear();
        base = mysets[(setNo)%numSets].begin()->first;
        base_phys = mysets[(setNo)%numSets].begin()->second;
        std::set<addrpair>::iterator startIt = mysets[setNo].begin();
        startIt ++ ;
        for (std::set<addrpair>::iterator it = startIt ; it != mysets[setNo].end() ; it ++)
        {
            first = it->first;
            first_phys = it->second;
            sched_yield();
        ...
        for (int i = 0 ; i < MAX_HIST_SIZE ; i ++) hist[i] = 0;
